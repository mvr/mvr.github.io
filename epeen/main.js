...
Elm.Main.make = function (_elm) {
   ...
   var choosePicture = function (mmr) {
      return function () {
         switch (mmr)
         {case 322:
            return $Maybe.Just("images/solo.png");
            case 9001:
            return $Maybe.Just("images/vegeta.png");}
         return _U.cmp(mmr,
         3000) < 0 ? $Maybe.Nothing : function () {
            var _v1 = A2($Basics._op["%"],
            mmr,
            7);
            switch (_v1)
            {case 0:
               return $Maybe.Just("images/bruno.png");
               case 1:
               return $Maybe.Just("images/godz.png");
               case 2:
               return $Maybe.Just("images/ld.png");
               case 3:
               return $Maybe.Just("images/basskip.png");
               case 4:
               return $Maybe.Just("images/synderen.png");
               case 5:
               return $Maybe.Just("images/dendi.png");
               case 6:
               return $Maybe.Just("images/2gd.png");}
            _U.badCase($moduleName,
            "between lines 88 and 95");
         }();
      }();
   };
   var mmrToInch = function (mmr) {
      return A2($Basics.clamp,
      0,
      20)((mmr - 3586) / 627 * 1.04 + 5.57);
   };
   var smoothFollow = function (s) {
      return function () {
         var towards = F2(function (_v2,
         current) {
            return function () {
               switch (_v2.ctor)
               {case "_Tuple2":
                  return current + (_v2._0 - current) * 1.2 * $Time.inSeconds(_v2._1);}
               _U.badCase($moduleName,
               "on line 70, column 56 to 109");
            }();
         });
   ...
   var mmrChannel = $Signal.channel("");
   var mmrSignal = $Signal.map(fromOk)(A2($Signal.keepIf,
   isOk,
   $Result.Ok(0))($Signal.map($String.toInt)($Signal.map(function (s) {
      return _U.eq(s,"") ? "0" : s;
   })($Signal.subscribe(mmrChannel)))));
   var inchesSignal = smoothFollow(A2($Signal.map,
   function ($) {
      return mmrToInch($Basics.toFloat($));
   },
   mmrSignal));
   var footer = A2($Html.div,
   _L.fromArray([$Html$Attributes.id("footer")]),
   _L.fromArray([$Html.text("Sources:")
                ,A2($Html.br,
                _L.fromArray([]),
                _L.fromArray([]))
                ,A2($Html.a,
                _L.fromArray([$Html$Attributes.href("http://www.reddit.com/r/DotA2/comments/2124az/ranked_mmr_survey_results_update/")]),
                _L.fromArray([$Html.text("What-A-Baller\'s MMR survey")]))
                ,A2($Html.br,
                _L.fromArray([]),
                _L.fromArray([]))
                ,A2($Html.a,
                _L.fromArray([$Html$Attributes.href("http://www.ncbi.nlm.nih.gov/pubmed/23841855")]),
                _L.fromArray([$Html.text("Herbenick et al. (2013)")]))]));
   var display = function (inches) {
      return A2($Html.div,
      _L.fromArray([]),
      _L.fromArray([A2($Html.div,
                   _L.fromArray([$Html$Attributes.id("inchResult")]),
                   _L.fromArray([$Html.text(A2($Basics._op["++"],
                   $String.left(4)($Basics.toString(inches)),
                   "in"))]))
                   ,A2($Html.div,
                   _L.fromArray([$Html$Attributes.id("ruler")
                                ,$Html$Attributes.style(_L.fromArray([{ctor: "_Tuple2"
                                                                      ,_0: "width"
                                                                      ,_1: A2($Basics._op["++"],
                                                                      $Basics.toString(inches),
                                                                      "in")}]))]),
                   _L.fromArray([]))]));
   };
   var mmrInput = function (mmr) {
      return A2($Html.input,
      _L.fromArray([$Html$Attributes.id("mmrInput")
                   ,$Html$Attributes.placeholder("Enter your MMR")
                   ,$Html$Attributes.value(_U.eq(mmr,
                   0) ? "" : $Basics.toString(mmr))
                   ,A3($Html$Events.on,
                   "input",
                   $Html$Events.targetValue,
                   $Signal.send(mmrChannel))]),
      _L.fromArray([]));
   };
   var scene = F2(function (mmr,
   inches) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.id("content")]),
      _L.fromArray([A2($Html.h1,
                   _L.fromArray([$Html$Attributes.id("heading")]),
                   _L.fromArray([$Html.text("Epeen Calculator")]))
                   ,mmrInput(mmr)
                   ,_U.cmp(inches,
                   0) > 0 ? display(inches) : $Html.text("")
                   ,function () {
                      var _v12 = choosePicture(mmr);
                      switch (_v12.ctor)
                      {case "Just":
                         return A2($Html.img,
                           _L.fromArray([$Html$Attributes.src(_v12._0)
                                        ,$Html$Attributes.$class("wow")]),
                           _L.fromArray([]));
                         case "Nothing":
                         return $Html.text("");}
                      _U.badCase($moduleName,
   ...
   var sceneElement = F3(function (mmr,
   inches,
   _v14) {
      return function () {
         switch (_v14.ctor)
